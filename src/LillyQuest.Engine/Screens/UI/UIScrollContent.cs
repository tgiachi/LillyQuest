...
public sealed class UIScrollContent : UIScreenControl
{
    ...
    public Rectangle<float> GetHorizontalThumbRect()
    {
        ...
        return new(thumbX, trackY, thumbWidth, ScrollbarThickness);
    }
    ...
    public Rectangle<float> GetVerticalThumbRect()
    {
        ...
        return new(trackX, thumbY, ScrollbarThickness, thumbHeight);
    }
    ...
    public Rectangle<float> GetViewportBounds()
    {
        ...
        return new(world.X, world.Y, width, height);
    }
    ...
    public override bool HandleMouseWheel(Vector2 point, float delta)
    {
        ...
        {
            ScrollOffset = new(ScrollOffset.X, ScrollOffset.Y + delta * ScrollSpeed);
        }
        else if (EnableHorizontalScroll && ContentSize.X > viewport.Size.X)
        {
            ScrollOffset = new(ScrollOffset.X + delta * ScrollSpeed, ScrollOffset.Y);
        }
        else
        {
            return false;
    ...
    )
    {
        ...
        spriteBatch.Draw(
            texture,
            new(dest.Origin.X, dest.Origin.Y),
            new(dest.Size.X, dest.Size.Y),
            tint,
            0f,
            Vector2.Zero,
            uv,
