...
public class TileSetServiceTests
{
    ...
    {
        var tileSetService = new TileSetService(new());
    ...
    public async Task TryGetTile_ReturnsFalse_WhenColorUnresolved()
    {
        ...
        await colorService.LoadDataAsync(
            new()
            {
                new ColorSchemaDefintionJson { Id = "schema", Colors = new() }
            }
        );
        ...
        await tileSetService.LoadDataAsync(
            new()
            {
                new TilesetDefinitionJson
                {
                    ...
                    Tiles = new()
                    {
                        new() { Id = "t1", Symbol = ".", FgColor = "fg", BgColor = "bg" }
                    }
                }
            }
        );
    ...
    {
        var tileSetService = new TileSetService(new()) { DefaultTileset = "main" };
        await tileSetService.LoadDataAsync(
            new()
            {
                new TilesetDefinitionJson { Name = "main", Tiles = new() }
            }
        );
    ...
    {
        var tileSetService = new TileSetService(new()) { DefaultTileset = "missing" };
        ...
        await tileSetService.LoadDataAsync(
            new()
            {
                new TilesetDefinitionJson
                {
                    ...
                    Tiles = new()
                    {
                        new() { Id = "t1", Symbol = ".", FgColor = "", BgColor = "" }
                    }
                }
            }
        );
