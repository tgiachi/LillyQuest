...
public class TilesetSurfaceTextExtensionsTests
{
    ...
    [Test]
    public void EnqueueMove_ActivatesAndClearsSource()
    {
        var screen = new TilesetSurfaceScreen(new StubTilesetManager());
        screen.InitializeLayers(1);
        screen.AddTileToSurface(0, 1, 1, new(5, LyColor.White));

        var result = screen.EnqueueMove(
            0,
            new(1, 1),
            new(2, 2),
            1f
        ...
        Assert.That(screen.GetTile(0, 1, 1).TileIndex, Is.EqualTo(-1));
        Assert.That(screen.GetActiveMovements(0).Count, Is.EqualTo(1));
    }

    [Test]
    public void EnqueueMove_FailsWhenSourceEmpty()
    {
        var screen = new TilesetSurfaceScreen(new StubTilesetManager());
        screen.InitializeLayers(1);
    ...
    public void LayerRenderScaleSmooth_MovesTowardsTarget()
    {
        ...
        screen.SetLayerRenderScaleTarget(0, 2f, 10f);
    ...
    public void LayerRenderScaleSmoothing_DisablesSmoothUpdates()
    {
        ...
        screen.SetLayerRenderScale(0, 1f);
        screen.SetLayerRenderScaleTarget(0, 2f, 10f);
        screen.SetLayerRenderScaleSmoothing(0, false, 1f);
    ...
    public void MouseCallbacks_BroadcastTileCoordinatesForAllLayers()
    {
        ...
        screen.SelectedLayerIndex = 0;
        screen.SetLayerInputTileSizeOverride(0, new Vector2(10, 10));
        screen.SetLayerInputTileSizeOverride(1, new Vector2(20, 20));
        screen.SetLayerPixelOffset(0, Vector2.Zero);
        screen.SetLayerPixelOffset(1, Vector2.Zero);
        screen.SetLayerViewTileOffset(0, Vector2.Zero);
        screen.SetLayerViewTileOffset(1, Vector2.Zero);
        screen.SetLayerViewPixelOffset(0, Vector2.Zero);
        screen.SetLayerViewPixelOffset(1, Vector2.Zero);
    ...
    public void MouseCallbacks_ReportTileCoordinates()
    {
        ...
        screen.SelectedLayerIndex = 0;
        screen.SetLayerInputTileSizeOverride(0, new Vector2(10, 10));
        screen.SetLayerPixelOffset(0, new(5, 5));
        screen.SetLayerViewTileOffset(0, Vector2.Zero);
        screen.SetLayerViewPixelOffset(0, Vector2.Zero);
        ...
        screen.OnMouseMove(25, 35);
        screen.OnMouseDown(25, 35, new[] { MouseButton.Left });
    ...
    [Test]
    public void MovementTilePosition_InterpolatesActiveAndReturning()
    {
        var screen = new TilesetSurfaceScreen(new StubTilesetManager());
        ...
        {
            ElapsedSeconds = 0.5f,
            State = TileMovementState.Active
        };

        var activePosition = screen.GetMovementTilePosition(movement);
        Assert.That(activePosition.X, Is.EqualTo(1f));

        movement.State = TileMovementState.Returning;
        var returnPosition = screen.GetMovementTilePosition(movement);
        Assert.That(returnPosition.X, Is.EqualTo(1f));
    }

    [Test]
    public void ProcessMovements_BounceReturnsToSource()
    {
        var screen = new TilesetSurfaceScreen(new StubTilesetManager());
        screen.InitializeLayers(1);
        screen.AddTileToSurface(0, 1, 1, new(5, LyColor.White));

        screen.EnqueueMove(0, new(1, 1), new(2, 2), 0.1f, true);
    ...
    [Test]
    public void ProcessMovements_CompletesAndPlacesTile()
    {
        var screen = new TilesetSurfaceScreen(new StubTilesetManager());
        screen.InitializeLayers(1);
        screen.AddTileToSurface(0, 1, 1, new(5, LyColor.White));

        screen.EnqueueMove(0, new(1, 1), new(2, 2), 0.1f);
        ...
        Assert.That(screen.GetActiveMovements(0).Count, Is.EqualTo(0));
    }
    ...
    public void RightClick_CentersViewOnTile()
    {
        ...
        screen.OnMouseDown(125, 105, new[] { MouseButton.Right });
    ...
    public void SmoothView_MovesTowardsTarget()
    {
        ...
        screen.SetLayerViewTileOffset(0, Vector2.Zero);
        screen.SetLayerViewTileTarget(0, new(4, 0));
    ...
    [Test]
    public void TileLayer_InitializesMovementQueue()
    {
        var screen = new TilesetSurfaceScreen(new StubTilesetManager());
        screen.InitializeLayers(1);

        var queue = screen.GetLayerMovementQueue(0);
