...
public class UIWindowTests
{
    ...
    public void ResizeHandle_Drag_Resizes_Window()
    {
        var window = new UIWindow
        {
            ...
            IsResizable = true,
            ResizeHandleSize = new Vector2(10, 10)
        };
    ...
    public void Resize_Clamps_To_Min_And_Max_Size()
    {
        ...
        window.HandleMouseUp(new Vector2(200, 200));

        Assert.That(window.Size, Is.EqualTo(new Vector2(120, 60)));

        start = new Vector2(window.Position.X + window.Size.X - 1, window.Position.Y + window.Size.Y - 1);
